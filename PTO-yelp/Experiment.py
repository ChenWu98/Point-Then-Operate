...
class Experiment(object):
    ...
    def __init__(self):
        ...
        print('----- Loading data -----')
        self.train_set = Yelp('train', False)
        self.test_set = Yelp('test', False)
        self.val_set = Yelp('dev', False)
        ...
        if config.train_mode == 'pto':
            ...
            self.train_set = Yelp('train', True)
            self.test_set = Yelp('test', True)
            self.val_set = Yelp('dev', True)
            self.best_acc = 0
        elif config.train_mode == 'cls-only':
            self.best_acc = 0
        self.best_metric = 0.30
