...
class InsFront(nn.Module):
    ...
    def forward(self, ori, pos, seq_len, sample):
        ...
        if_lgt = self.MLP(gthr_hid)  # shape = (n_batch, voc_size)
        if_prob = F.softmax(if_lgt, dim=1)  # shape = (n_batch, voc_size)
        if not sample:
            # Choose max.
            sample_prob, sample_idx = torch.max(if_prob, dim=1)  # (n_batch, ), (n_batch, )
        else:
            # Sample by multinomial distribution.
            sample_idx, sample_prob = sample_2d(probs=if_prob, temperature=self.temperature)
        _if = torch.zeros_like(ori).copy_(ori)  # shape = (n_batch, 20)
        # i  i+1  i+2  ... ->
        #    i    i+1  i+2 ...
        for b in range(B):
            _if[b, pos[b]+1:] = _if[b, pos[b]:-1]
        _if.scatter_(dim=1,
                     index=pos.unsqueeze(1),  # shape = (n_batch, 1)
                     src=sample_idx.unsqueeze(1)  # shape = (n_batch, 1)
                     )  # shape = (n_batch, 20)

        return _if, sample_prob, if_lgt